# Untitled - By: marie - Wed Apr 16 2025
...
while True:
    ...
    if waiting_for_card:
        ...
        #set up next round
        count += 1
        if count >= 3: #end game after 5 rounds
            #if len(player_list) > 1 and suit_colour[player_list[-1]] == suit_colour[player_list[-2]]: #if player doesn't change card colour
              #  player_score = -1 #penalty incurred
               # print('Never changed card colour, penalty!')
        ...
        for i in range(3, 0, -1): #pause for 3 seconds, player must change card
            print(i)
            time.sleep(1)
