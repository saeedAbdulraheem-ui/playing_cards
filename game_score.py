...
print('System card:', system_suit) #print card for system
label_buffer = []

#function to give camera 5 secs to register card
  ...
  #  label, conf = max(predictions_list, key=lambda x: x[1])
    ...
    if waiting_for_card:
        if top_score > 0.5 and start_time is None: #if start time is none and confidence iterval is above 50%
            ...
            if time.time() - start_time > 7: # if 7 secs have based
                if len(label_buffer) > 0: #buffer list is not empty
                    my_label = max(set(label_buffer), key=label_buffer.count)  #set the label as the most frequent label in the buffer list
